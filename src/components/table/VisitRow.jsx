...
export default function VisitRow({
    ...
    const handleFollowupChange = async (e) => {
        ...
        try {
            await updateVisit(row.id, { followup_type: newValue });
            refreshData();
        } catch (err) {
...
<td className="px-4 py-3 text-center relative group">
  {row.notes ? (
    <>
      ...
      {/* HOVER TOOLTIP */}
      <div className="absolute z-10 hidden group-hover:block bg-black text-white text-xs rounded px-2 py-1 -top-8 left-1/2 -translate-x-1/2 whitespace-nowrap shadow-lg">
        {row.notes}
      </div>
    </>
  ) : "-"}
